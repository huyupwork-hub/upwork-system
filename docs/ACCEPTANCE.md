...
| G5 | ☐ Output is credible enough to show a client. | No sample PDF committed yet. The document to commit is the reviewer's own download of a stored report, `docs/evidence/reports/fieldproof-northgate-retail-park-20260822-<id8>.pdf`, once DEPLOY §7 has run — a real generated document, not a fixture |
...
| G11 | ☐ The PDF the device renders at submission is stored once in the private `inspection-reports` bucket, and a reviewer downloads it through a signed URL (D21 amended, D31). | pgTAP `110` — bucket private, PDF-only, 50 MB; the owner's first insert lives · hosted smoke **22a** — the app's own `ReportService.publish` lands `{uid}/{id}/report.pdf`, `published()` lists it, the signed URL serves 200 `application/pdf` starting `%PDF-`, the unsigned URL does not · `repository.test.ts` — the folder is listed, `createSignedUrl` at 600 s with `download:` the product filename · `render.test.tsx` — `report-link` · run id pending (slice section below) |
| G12 | ☐ A stored report cannot be replaced, altered or removed by any client, a draft cannot have one, and the console cannot write one. | pgTAP `110` — `23505` on the second insert at the pinned name; `42501` × 10 (sibling, deeper path, draft, foreign id, forged owner segment, photo bucket, B forging, admin twice, anon); silent UPDATE followed by a privileged re-read; zero UPDATE/DELETE policies name the bucket; the planted draft object and sibling stay invisible to the admin · hosted smoke **22b–22d** — `Duplicate` on re-upload, `upsert: true` refused, `remove()` returns `[]` and the object is still listed, B cannot list, sign or upload, a draft is refused at the server · `AdminRepository` still has no write method and `render.test.tsx` still forbids any control · run id pending |
| G13 | ☐ A failed upload never un-submits or misreports the submission, is retryable from the record and from the Reports tab, and "could not check" is never shown as "not uploaded". | `report_publish_flow_test.dart` (17 cases) — a `SocketException` after the submit shows `report-publish-error` and `publish-report-button`, no `submit-error`, status still `Submitted`, `submitted == ['insp-1']`; the retry lands the object; `failPublished` shows `report-state-unknown` with a retry and the findings still render; the Reports tab reads per-row state from the bucket, `upload-missing-reports-button` publishes only the missing ones and `reports-publish-summary` names them; a fresh widget tree over the same store shows the same state · `report_store_test.dart` (26 cases) — `publishMissing` stops at the first transport failure and reports the rest as skipped, continues past a refusal, never throws, uploads nothing on a second run · run id pending |
...
| I8 | ☐ A reviewer is told, per inspection, whether a report exists, could not be loaded, or is absent — and the queue prints a dash when it did not ask. | `render.test.tsx` — `report-link`, `report-unloadable` (*The PDF report exists but could not be loaded.*) and `report-absent` (*No PDF report has been uploaded for this inspection yet…*) are three different renderings and never coincide; the queue's Report column reads `PDF` / `Not yet` / `—` for `hasReport` true / false / undefined · `density.test.tsx` — the absent copy carries no error word · `repository.test.ts` — `{present: true, url: null}` when signing fails, `{present: false, url: null}` with no signing call when the folder is empty, a folder that cannot be listed rejects rather than reading as absent, and `hasReport` stays undefined when an inspector's folder listing errors, throws, or fills its page (D28: a dash, not a claim) · run id pending |
...
| K3 | ◐ pgTAP suite passes against a clean migration run — **12 files**, `010`–`110`. | Last green: run `33935381654` ✅ — `Files=11, Tests=208`, `Result: PASS`, three times (from empty, from seed, re-apply). `110_inspection_reports.test.sql` adds 28 assertions in the stored-reports slice and has not been through CI yet, so this line is ◐ until that run id is recorded in the slice section below |
...
| M3 | ☐ A sample generated PDF is committed. | `docs/evidence/reports/fieldproof-northgate-retail-park-20260822-<id8>.pdf` — the stored report, downloaded as the reviewer through the signed URL (DEPLOY §7); closes with G5 |
| M4 | ☐ No criterion above is claimed without its named evidence. | This document, fully ☑ |
| M5 | ☐ The demo admin opens the PDF of each of the three demo inspections. | `docs/evidence/admin/` — one screenshot per detail page showing **Download the PDF report**, plus the `storage.objects` read-back in DEPLOY §7 (exactly three rows, `<demo uid>/<id>/report.pdf`). The read-back closes it, not the taps that produced the objects |
...
*Amended 2026-09-05 (D31): the paragraph above describes the slice as shipped at run
`33432719089`. One rendering is now uploaded, write-once, at submission, and the console
links to it — G11–G13 and the stored-reports section below. The "no report table" clause,
and everything about who renders, still hold.*
...
*2026-09-05: D31 amends the second clause — the device's rendering is now stored once at
submission and the console links to it through a signed URL on the reviewer's session.
Nothing about who renders, and nothing about the console's privilege, changed (I8).*
...
## Slice in progress — Stored reports (D21 amended, D31)

**Status: built, awaiting its run ids.** `submitted inspection → PDF rendered on the
device → uploaded once, write-once, to the private inspection-reports bucket → signed URL in
the console`. Nothing here is ☑ until the CI run that carries it is recorded; the numbers
below are what the tree holds, read locally, and CI is the arbiter (D1).

| Gate | What it carries |
|---|---|
| Database + RLS | `20260905001100_inspection_reports.sql` — the eleventh migration, from empty and from seed; pgTAP `010`–`110`, **12 files**, `110` adding 28 assertions; `070`/`080` byte-identical to before |
| Mobile (Flutter) | `ReportStore` port with one production store and one fake; `ReportService.publish` / `published` / `publishMissing`; upload state on the detail screen and the Reports tab; `flutter test` **323** on this tree, 44 of them new (`report_store_test.dart` 26, `report_publish_flow_test.dart` 17, one cap-parity case) |
| Admin (Next.js) | folder listing and a 600 s signed URL with the product filename in `AdminRepository`; three report states in the detail view; a Report column in the queue; vitest **58 tests / 7 files**, `report_filename.test.ts` pinning the two Dart filename fixtures |
| Hosted smoke | cases **22a–22e** inserted after 22, case 38 extended; the manifest's `reportPaths`; the purge removing the report object before the rows and ending `nothing named remains` |
| Secret hygiene | no new key anywhere — the store, the console and the backfill runner hold the publishable key only |

**Turned to ☑ by the release pass** (DEPLOY §3, §4, §7), which records the CI run id, the
Vercel deployment id, `v0.1.0-demo.3`, and the backfill read-back: G11–G13, I8, K3, M5 —
and G5/M3 with the Northgate download.

### Deliberately not done

No report table, no server-side render, no hash the console verifies, no automatic upload
sweep on app open or resume, no orphan reclaim, no byte-equality assertion between a stored
rendering and a fresh one (the footer carries `generatedAt`; the record is authoritative),
and no client delete path for a stored report — the service role in the smoke purge is the
only one.
